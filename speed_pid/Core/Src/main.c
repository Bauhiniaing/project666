...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : 电机速度闭环控制主程序（带死区防震荡）
  * @note           : Generated by STM32 PID Generator
  ******************************************************************************
  * @attention
  *
...
#define PID_DEFAULT_KI       0.1f     // 默认积分系数
#define PID_DEFAULT_KD       10.0f     // 默认微分系数
#define PID_MAX_OUTPUT       16384.0f  // PID最大输出
#define PID_MAX_INTEGRAL     2000.0f  // PID最大积分值
#define PID_DEADBAND         0.0f     // PID误差死区阈值
/* USER CODE END PD */
...
static PID_HandleTypeDef hpid_speed;          // 速度PID句柄

static int32_t target_speed = 0;            // 目标速度值
/* USER CODE END PV */
...
int main(void)
{
  ...
  {
    // PID速度闭环计算
    float pid_output = PID_Calculate(&hpid_speed, (float)target_speed, (float)hmotor.speed_rpm);
...
static float PID_Calculate(PID_HandleTypeDef *hpid, float target, float feedback)
{
  ...
  // PID核心计算
  hpid->output = hpid->Kp * hpid->error +
                 hpid->Ki * hpid->integral +
                 hpid->Kd * (hpid->error - hpid->last_error);
